...
// export const addAccident = async (req: Request, res: Response) => {
//   try {
//     const { date, deaths, injuries, position, level, cause, link } = req.body;
//     if (!date || !deaths || !injuries || !position || !level || !cause || !link) {
//       return res.status(400).send("ERROR ADD ACCIDENT: Missing required fields");
//     }
//     const newAccident = await AccidentModel.create(req.body);
//     return res.status(200).json({ accident: newAccident });
//   } catch (err) {
